...
import { Send, Sparkles, User, Plus, Calendar, AlertCircle, Search, Settings2 } from "lucide-react";
...
export default function ChatInterface({ 
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "ai",
      content: "Hello. I am your AI Event Assistant. I can help you discover upcoming festivals and markets in the Perth area based on your selected criteria. How can I assist you today?"
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setMessages(prev => [...prev, { 
          role: "ai", 
          content: res.message || res.verbalResponse || (res.data.length > 0 ? "I have identified the following events matching your search criteria:" : "I could not find any events matching those criteria at this time."),
          events: res.data 
        }]);
      }
    } catch (err) {
      ...
      <div className="p-4 border-b border-outline-variant/15 flex items-center justify-between bg-surface-container-low/30 backdrop-blur-md sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-black text-on-surface uppercase tracking-widest truncate">AI Event Assistant</h2>
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 rounded-full bg-primary" />
              <span className="text-[10px] font-bold text-on-surface-variant/40 uppercase tracking-tighter truncate">Service Active • Perth</span>
            </div>
          </div>
        </div>
          ...
          <div key={idx} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`flex gap-3 max-w-[85%] ${m.role === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center border shadow-sm ${
                ...
                {m.role === "user" ? <User size={14} /> : m.error ? <AlertCircle size={14} /> : <Sparkles size={14} strokeWidth={2} />}
              </div>
                      ...
                      <div key={eIdx} className="bg-surface p-4 rounded-3xl border border-outline-variant/20 shadow-sm group hover:border-primary/40 transition-all">
                        <div className="flex items-start justify-between mb-2">
                          <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${
                            ev.type === "festival" ? "bg-primary/10 text-primary" :
                        ...
                        <p className="text-[10px] text-on-surface-variant font-medium mb-2 truncate opacity-60">{ev.location}</p>
      ...
      <div className="p-4 md:p-6 border-t border-outline-variant/15 bg-surface-container-low/30">
        <div className="max-w-4xl mx-auto relative group">
          <textarea 
            ref={textareaRef}
            rows={1}
            placeholder={isTyping ? "Searching database..." : "Describe the events you are looking for..."}
            value={input}
