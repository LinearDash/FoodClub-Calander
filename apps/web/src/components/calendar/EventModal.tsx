            ...
            return (
              <div
                ...
                className={`h-6 text-[11px] rounded text-center leading-6 ${
                  inRange ? "bg-primary/20 text-on-surface font-semibold" : "text-on-surface-variant"
        ...
        {mode === "preview" ? (
            ...
            <div className="space-y-4">
              ...
              {event.followUpDate && (
                <div className="rounded-xl bg-surface-container-low p-3 text-sm">
                  ...
                  <p className="font-medium text-on-surface">{formatDate(event.followUpDate)}</p>
                </div>
              )}
              {(event.contactName || event.contactDetails) && (
                <div className="rounded-xl bg-surface-container-low p-3 text-sm">
                  ...
                  <p className="font-medium text-on-surface">
                    {event.contactName || event.contactDetails}
                    {event.contactName && event.contactDetails ? ` (${event.contactDetails})` : ""}
                  </p>
                </div>
              )}
              {event.notes && (
          ...
          <>
            <h3 className="font-display text-2xl font-bold text-on-surface">
              {event.id ? "Edit Event" : "New Event"} {event.isTBA ? "(TBA)" : ""}
            </h3>
            ...
            <div className="space-y-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="flex rounded-xl border border-outline-variant/30 overflow-hidden mb-3 w-fit">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setEvent({ ...event, applicationFormReleaseDateType: "range" })
                      }
                      className={`px-4 py-1.5 text-sm font-semibold transition-colors ${
                  ...
                  {(!event.applicationFormReleaseDateType ||
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEvent({ ...event, applicationFormReleaseDate: e.target.value })
                      }
              ...
              <div>
                <label className="block text-xs font-semibold uppercase text-on-surface-variant mb-1">
                  Attachment{" "}
                  {isUploading && (
                    <span className="text-primary animate-pulse ml-2 font-bold">(Uploading...)</span>
                  )}
                </label>
                {!event.id ? (
                ...
                {event.documents && (event.documents as string[]).length > 0 && (
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={!!event.isTBA}
                  onChange={(e) => setEvent({ ...event, isTBA: e.target.checked })}
                  className="w-4 h-4 text-primary rounded border-outline-variant/30 focus:ring-primary"
                />
                <span className="text-sm font-medium text-on-surface">Mark as TBA Event (No fixed date)</span>
              </label>
            </div>
